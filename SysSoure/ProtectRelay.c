...
extern void ProtectRlyHandle(PrtectRelayReg *P);
...
void ProtectRlyHandle(PrtectRelayReg *P)
{
    ...
    {
        P->WakeupOff_TimeCount=0;
        if(P->State.bit.WakeupOnTiemrErr==0)
        {
            P->WakeupOn_TimeCount++;
            ...
            if(P->WakeupOn_TimeCount>PrtctWakeupOnTimeOut)
            {
                P->WakeupOn_TimeCount=PrtctWakeupOnTimeOut+10;
                P->State.bit.WakeupOnTiemrErr=1;
            }
            if(P->State.bit.WakeuPOnEND==0)
            {
                P->State.bit.PRlyDO   = P->State.bit.CANPRlyDO;
                P->State.bit.NRlyDO   = P->State.bit.CANNRlyDO;
                P->State.bit.PreRlyDO = P->State.bit.CANPreRlyDO;
                P->State.bit.ProRlyDI = P->State.bit.PreRlyDO;
            }
        }
        else
        {
            P->State.bit.CANPRlyDO=0;
            P->State.bit.CANPRlyDO=0;
            P->State.bit.CANPRlyDO=0;
    ...
    else if(P->State.bit.WakeUpEN==0)
    {
        P->WakeupOn_TimeCount=0;

        if(P->State.bit.WakeupOFFTiemrErr==0)
        {
            P->WakeupOff_TimeCount++;
            ...
            if(P->WakeupOff_TimeCount>PrtctWakeupOFFTimeOut)
            {
                P->WakeupOff_TimeCount=PrtctWakeupOFFTimeOut+10;
                P->State.bit.WakeupOFFTiemrErr=1;
            }
            if(P->State.bit.WakeuPOffEND==0)
            {
                P->State.bit.PRlyDO   = P->State.bit.CANPRlyDO;
                P->State.bit.NRlyDO   = P->State.bit.CANNRlyDO;
                P->State.bit.PreRlyDO = P->State.bit.CANPreRlyDO;
